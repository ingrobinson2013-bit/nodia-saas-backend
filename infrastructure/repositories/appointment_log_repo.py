...
# Acceso a datos de citas_log — 100% PostgreSQL Nativo & Clean Architecture
...
class AppointmentLogRepository:
    ...
    def get_active_appointments(self, tenant_id: str, wa_from: str) -> List[dict]:
        sql = """
        SELECT id, odoo_event_id, fecha_cita, hora_cita, servicio, profesional, estado
        FROM citas_log 
        WHERE tenant_id = %s 
          AND wa_from = %s 
    ...
    def update_status_by_ids(self, ids: List[Any], estado: str) -> bool:
        ...
        return execute_sql(sql, (estado, ids))
    ...
    def update_status_by_id(self, cita_id: Any, estado: str) -> bool:
        ...
        return execute_sql(sql, (estado, cita_id))

    def cancel_appointment_by_odoo_id(self, tenant_id: str, odoo_event_id: Any) -> bool:
        sql = "UPDATE citas_log SET estado = 'cancelada' WHERE tenant_id = %s AND odoo_event_id = %s;"
        return execute_sql(sql, (tenant_id, str(odoo_event_id)))
    ...
    def cancel_all_appointments_by_phone(self, tenant_id: str, wa_from: str) -> bool:
        ...
        return execute_sql(sql, (tenant_id, wa_from))
    ...
    def get_future_appointments_by_tenant(self, tenant_id: str, since_date: str, limit: int = 30) -> List[dict]:
        sql = """
        SELECT fecha_cita, hora_cita, profesional, servicio 
        FROM citas_log 
        WHERE tenant_id = %s 
          AND estado != 'cancelada' 
          AND fecha_cita >= %s 
